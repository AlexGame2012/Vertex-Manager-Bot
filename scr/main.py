...
from config import BOT_TOKEN, PREFIXES, RANKS, MASTER_ID, CURRENCY_NAME, CURRENCY_EMOJI
...
apihelper.proxy = {'https': 'socks5://127.0.0.1:10808'} #Your proxy server (required for the bot to work from Russia)
...
def process_command(message, cmd, args):
    ...
    if cmd in ['дк', 'access']:
        ...
        bot.reply_to(message, 
            ...
            f"• Открыть лутбокс — `!лутбокс` (скоро)\n\n"
            f"**📤 Передать другу:** `!передатьвертексы 100 @user`", 
            parse_mode="Markdown")
        ...
        bot.reply_to(message, f"🌾 Вы собрали урожай! +{reward} {CURRENCY_NAME}\n"
                             f"📊 Бонус за активность: +{bonus}\n"
                             f"{CURRENCY_EMOJI} Ваш баланс: {db.get_vertexes(user_id)}")
